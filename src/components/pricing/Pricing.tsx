  ...
  const doctorPlans = [
    {
      icon: (
        <div className="bg-gradient-to-tr from-blue-100 to-purple-100 rounded-xl w-12 h-12 flex items-center justify-center mb-4">
          <svg width="28" height="28" fill="none"><circle cx="14" cy="14" r="12" fill="#6366F1"/></svg>
        </div>
      ),
      label: "للأطباء الجدد",
    ...
    },
    {
      icon: (
        <div className="bg-gradient-to-tr from-blue-100 to-purple-100 rounded-xl w-12 h-12 flex items-center justify-center mb-4">
          <svg width="28" height="28" fill="none"><rect x="4" y="4" width="20" height="20" rx="4" fill="#6366F1"/></svg>
        </div>
      ),
    ...
    },
    {
      icon: (
        <div className="bg-gradient-to-tr from-blue-100 to-purple-100 rounded-xl w-12 h-12 flex items-center justify-center mb-4">
          <svg width="28" height="28" fill="none"><polygon points="14,4 24,24 4,24" fill="#6366F1"/></svg>
        </div>
      ),
  ...
  return (
    <div className="min-h-screen flex flex-col items-center bg-gradient-to-tr from-blue-50 via-white to-purple-50 py-6 px-4">
      <div className="flex flex-col items-start w-full max-w-7xl mb-6">
        <h1 className="text-3xl md:text-4xl font-bold text-blue-700 mb-3 text-right">باقات الاشتراك</h1>
        <h2 className="text-lg md:text-xl text-gray-500 mb-8 text-right">اختر ما يناسبك</h2>
      </div>
      {/* أزرار التبديل بين الشهري والسنوي */}
      <div className="flex justify-center items-center gap-4 mb-4">
        <span className={`cursor-pointer font-semibold ${isMonthly ? 'text-blue-800' : 'text-gray-500'}`} onClick={() => setIsMonthly(true)}>شهريًا</span>
        <div className="w-12 h-6 bg-blue-200 rounded-full relative cursor-pointer" onClick={() => setIsMonthly(!isMonthly)}>
          <div className={`w-5 h-5 bg-white rounded-full absolute top-0.5 transition-all duration-300 ${isMonthly ? 'right-0.5' : 'left-0.5'}`}></div>
        </div>
        <span className={`cursor-pointer font-semibold ${!isMonthly ? 'text-blue-800' : 'text-gray-500'}`} onClick={() => setIsMonthly(false)}>سنويًا</span>
      </div>
      ...
      <div className="flex justify-center gap-2 mb-20">
        <button
          className={`px-6 py-2 rounded-full font-semibold transition ${activeTab === 'doctors' ? 'bg-blue-700 text-white' : 'bg-blue-100 text-blue-700'}`}
          onClick={() => setActiveTab('doctors')}
        >
          باقات الأطباء
        </button>
        <button
          className={`px-6 py-2 rounded-full font-semibold transition ${activeTab === 'patients' ? 'bg-blue-700 text-white' : 'bg-blue-100 text-blue-700'}`}
      ...
      {/* عرض الباقات */}
      <div className={`grid grid-cols-1 md:grid-cols-${plans.length} gap-8 max-w-6xl w-full`}>
        {plans.map((plan, idx) => (
          <div
          key={idx}
          className={`
            flex flex-col items-center rounded-3xl shadow-xl bg-white
            ${plan.popular ? "border-4 border-blue-600 scale-105 z-10 md:-mt-8 md:mb-8" : "border border-gray-100"}
        ...
        >
            ...
            <div className="text-gray-400 mb-4">{plan.period && `/${plan.period}`}</div>
            <div className="w-full text-right mb-2 font-bold">المميزات</div>
            <ul className="w-full mb-8 space-y-2">
              {plan.features.map((feature, i) => (
                <li key={i} className="flex items-center gap-2 text-blue-700 font-medium">
            ...
            <button
              className={`
                ...
                ${plan.popular ? "bg-blue-700 text-white hover:bg-blue-800" : "bg-blue-100 text-blue-700 hover:bg-blue-200"}
                transition
              `}
            >
              {plan.button}
            ...
            {plan.popular && (
              <span className="absolute top-6 left-6 bg-blue-600 text-white text-xs font-bold px-3 py-1 rounded-full shadow">الأكثر شيوعًا</span>
            )}
          </div>
        ))}
      </div>
