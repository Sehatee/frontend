...
function DirectCall() {
  ...
  function handleSubmit() {
    ...
    const checkboxElement = document.getElementById('agree') as HTMLInputElement;
  ...
  return (
      ...
      ></textarea>
      <p className="text-sm text-gray-500 mb-4">{t("char")}</p>
      ...
      <div className="flex items-start gap-2 mb-6">
        ...
        <label htmlFor="agree" className="text-sm leading-relaxed text-gray-700">
        {t("check")} 
        <button
          type="button"
      ...
      {/* زر تأكيد */}
      <button className="w-full bg-blue-600 hover:bg-blue-700 text-white text-lg py-3 rounded-xl transition" onClick={handleSubmit}>
      {t("submit")}
      </button>
        ...
        <div className="fixed top-[72px] left-0 right-0 flex items-center justify-center">
          <div className="bg-green-100 text-black p-4 rounded-md shadow-lg flex items-center space-x-2">
            ...
            <div>
              ...
              <p className="text-sm">سيتم التواصل معك قريبًا من قبل الطبيب المختص. شكرًا لثقتك بمنصتنا</p>
            </div>
            <button className="ml-auto text-gray-500 hover:text-gray-700" onClick={() => setShowToast(false)}>
   ...
   <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center ">
    <div className="bg-white max-w-2xl w-full max-h-[80vh] overflow-y-auto p-6 rounded-xl shadow-lg relative">
      ...
      <h2 className="text-xl font-bold text-center mb-4 text-main mt-2">{t("Termstitle")}</h2>
      <div className={`text-sm text-ft leading-loose whitespace-pre-wrap px-3`}>
        <p>
        {t("Termscontent")}
        </p>
      </div>
